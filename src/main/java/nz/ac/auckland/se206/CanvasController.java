...
public class CanvasController {
	...
	@FXML
	protected Label predictionLabel;
	...
	@FXML
	protected Button saveDrawingButton;
	 ...
	 */
	@FXML
	protected void onInitialize() throws ModelException, IOException, CsvException, URISyntaxException {
		initialize();
	}
	 ...
	 * @throws IOException        If the file is not found.
	 * @throws TranslateException If error is raised during processing of
	 *                            input/output
	 */
	@FXML
	...
	/**
	 * Checks if the player has won. The player wins if the computer guesses the
	 * word in their top 3 predictions
	 *
	...
	protected String getRandomWord() throws IOException, CsvException, URISyntaxException {
		...
		while (firstSelection == true || usedWords.contains(randomWord)) {
			...
			if (currentUser.getWordsSetting() == Level.EASY) {
				...
				firstSelection = false;
				String[] possibleWords = new String[] { categorySelector.getRandomCategory(Difficulty.E),
	...
	/**
	 * Updates the user's fastest win time if the current win time is faster than
	 * the record time.
	 *
	 * @throws Exception General exception
	 */
	...
	/**
	 * Saves any stats data. This is a manual save that is performed via a button
	 * (as it's going to be very time consuming to write the save contents all the
	 * time)
	 *
	 * @throws Exception General exception
	 */
			...
			for (String key : usersHashMap.keySet()) {
				if (isFirst == true) {
					...
					if (usersHashMap.get(key).getUsedWords().isEmpty()) {
						...
						bf.write(usersHashMap.get(key).getSaveDetails() + ":"
								+ usersHashMap.get(key).formatWordsForSave(usersHashMap.get(key).getUsedWords()));
					}
					isFirst = false;
				} else {
					break;
	...
	private void updateBadges(Integer winTime) throws Exception {
		...
		for (Boolean badge : badgesArray) {
			if (badge == false) {
				...
				if (currentUser.getAccuracySetting() == Level.HARD && currentUser.getConfidenceSetting() == Level.MASTER
				...
				if (currentUser.getAccuracySetting() == Level.HARD && currentUser.getConfidenceSetting() == Level.HARD
						&& currentUser.getTimeSetting() == Level.HARD && currentUser.getWordsSetting() == Level.HARD) {
