...
public class MenuController {
	...
	@FXML
	private Text playerFiveText;

	@FXML
	private Text playerSixText;

	@FXML
	private Text playerSevenText;

	@FXML
	private Text playerEightText;
	...
	@FXML
	private Button playerFiveButton;

	@FXML
	private Button playerSixButton;

	@FXML
	private Button playerSevenButton;

	@FXML
	private Button playerEightButton;
	...
	/**
	 * Gets the difficulty string from file, and puts the difficulty settings into
	 * an array
	 *
	...
	/**
	 * Gets the saved users data from file and loads this data onto the users hash
	 * map. This is done before every register/login operation
	 *
	 * @throws IOException If the file is not found.
	 */
	private void loadUsers() throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			insertUser.loadUser(parts[0], Integer.valueOf(parts[1]), Integer.valueOf(parts[2]),
					Integer.valueOf(parts[3]), Integer.valueOf(parts[4]),
					loadDifficultySettingsFromFile(parts[5]).get(0), loadDifficultySettingsFromFile(parts[5]).get(1),
					loadDifficultySettingsFromFile(parts[5]).get(2), loadDifficultySettingsFromFile(parts[5]).get(3),
	...
	private void playSound(String fileName) throws URISyntaxException {
	...
	/**
	 * Handles registration of new users. Will check if their username exists in the
	 * hashmap, and if not, it registers them.
	 *
	 * @throws IOException If the file is not found.
	 */
	@FXML
	...
	/**
	 * For log in functionality - checks if a username exists in the hashmap, and if
	 * it does, it logs them in (displays their main stats).
	 *
	 * @throws IOException If the file is not found.
	 */
	@FXML
